...
```{r lce_residual_fitted, include=FALSE}
df_lce %>% 
...
```{r lce_residual_rate, include=FALSE}
df_lce %>% 
...
```{r lce_residual_length, include=FALSE}
df_lce %>% 
...
```{r non_constant_variance, include=FALSE}
df_lce %>% 
  ggplot(mapping = aes(x = predicted, y = residuals)) + 
  geom_point() +
  labs(title = 'Residuals vs. Predicted', 
...
> Based on the histogram and Q-Q plot, we see a residual distribution that is pretty faithful to the normal distribution. We conclude that this assumption is met.
