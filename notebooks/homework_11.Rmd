...
**Response**: Companies with high value assets are likely the ones experiencing the most attention from cybercriminals, and so to remedy this they use their greater means to invest in cybersecurity training. The causal sequence might look like the following: 
...
\begin{tikzpicture}
  % nodes %
  \node[ellipse, draw, align=center] (A) {Org has\\valuable\\assets};

  \node[ellipse, draw, right=0.25 of A, align=center] (B) {Org becomes\\target of\\cybercrime};

  \node[ellipse, draw, right=0.25 of B, align=center] (C) {In response,\\Org must perform\\effective mitigation\\to protect assets};

  \node[ellipse, draw, right=0.25 of C, align=center] (D) {Org invests in\\cybersecurity\\training};
  
  \node[ellipse, draw, right=0.25 of D, align=center] (E) {Relative drop\\in security\\breaches};

  \node[ellipse, draw, below=0.5 of C, align=center] (Y) {Number of\\Security\\Breaches};
  ...
  \draw[->] (C) -- (D);
  \draw[->] (D) -- (E);
  \draw[->] (B) edge node[draw, fill=white, circle]{+} (Y);
  \draw[->] (E) edge node[draw, fill=white, circle]{-} (Y);
\end{tikzpicture}
\end{center}

Since these organizations are investing in training as a response to being targeted, we expect the direction of bias introduced into the model to be away from zero in the positive direction. Our colleague is misinterpreting a reverse causality in these model results to suggest:
...
**Response**: Emails Encrypted is a reflection of how well employees at large comply internalize good cybersecurity practices, which itself is an outcome of having received adequate cybersecurity training. Removing the Emails Encrypted variable will make the coefficient on cybersecurity training hours *move down* since it is currently absorbing much of the positive effect that the training has on employees. Much of the impact of having *well-trained* employees is not being counted towards the coefficient for training hours if we are additionally accounting for the best practices directly through Emails Encrypted. 
